...
class AutoUpgrader {
    ...
    upgradeAllTables(opts) {
        return this.upgradeTables((0, metadata_1.schema)().getAllTables(), opts);
    }
    /*
        ...
        return __awaiter(this, void 0, void 0, function* () {
            ...
            try {
                if (Array.isArray(tables)) {
                    yield (0, utils_1.sequentialize)(tables.map((table) => () => this._upgradeTable(table, opts)));
                }
                else {
    ...
    _getUpgradeInfo(table, tableInfo, opts) {
        ...
        for (const name of Object.keys(tableInfo.indexes)) {
            const idx = table.mapNameToIDXDef.get((0, utils_1.qualifiySchemaIdentifier)(name, tableInfo.schemaName));
            if (!idx) {
    ...
    createTable(table) {
        ...
        return (0, utils_1.sequentialize)(factories).then(() => { });
    }
    /*
    ...
    alterTable(table, upgradeInfo) {
        ...
        Object.keys(tableInfo.indexes).forEach((name) => {
            ...
            if (!idx) {
                ...
                factories.push(() => this.sqldb.exec(`DROP INDEX IF EXISTS ${(0, utils_1.quoteIdentifier)(name)}`));
    ...
    recreateTable(table, upgradeInfo) {
        ...
        const tmpTableName = (0, utils_1.quoteIdentifier)(table.name + '_autoupgrade');
        // rename old table
        ...
        if (keepOldColumns) {
            colNames = Object.keys(tableInfo.columns)
                .map((colName) => (0, utils_1.quoteIdentifier)(colName))
                .join(',');
        }
        else {
            colNames = Object.keys(tableInfo.columns)
...
FROM  ${tmpTableName}`;
        ...
        return this.sqldb.transactionalize(() => (0, utils_1.sequentialize)(factories).then(() => { }));
    }
    /*
