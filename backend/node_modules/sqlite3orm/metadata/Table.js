...
class Table {
    ...
    getCreateIndexStatement(idxName, unique) {
        ...
        return ('CREATE ' +
            (unique ? 'UNIQUE ' : ' ') +
            `INDEX IF NOT EXISTS ${(0, utils_1.quoteIdentifier)(idxName)} ON ${(0, utils_1.quoteAndUnqualifyIdentifier)(this.name)} ` +
            `(` +
            idxCols.join(', ') +
            ')');
    }
    ...
    createCreateTableStatement(force, addFields) {
        ...
        this.mapNameToFKDef.forEach((fk, fkName) => {
            ...
            stmt +=
                fk.fields.map((field) => (0, utils_1.quoteSimpleIdentifier)(field.foreignColumnName)).join(', ') +
